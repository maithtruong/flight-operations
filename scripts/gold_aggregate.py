...
def run_gold_aggregate(context):
    ...
    states_df = pd.concat(
        [pd.read_csv(f) for f in silver_files], ignore_index=True
    )
    states_df["retrieved_time"] = pd.to_datetime(
        states_df["retrieved_time"], utc=True
    )
    ...
    gold_df = pd.DataFrame([{
        ...
        "mode_country":   states_df["pos_country"].mode().iloc[0]
                          if not states_df["pos_country"].mode().empty else None,
        "mode_city":      states_df["pos_city"].mode().iloc[0]
                          if not states_df["pos_city"].mode().empty else None,
    }])
